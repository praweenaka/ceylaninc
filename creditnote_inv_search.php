    ...
    <div id="filt_table" class="CSSTableGenerator container"> 
        <table id="testTable"  class="table table-bordered">
            <?php
             ...
             $sql = "select REF_NO, SDATE,C_CODE,CUS_NAME,GRAND_TOT from s_salma where trn_type ='INV' and CANCELL='0' ORDER BY id desc ";
            ...
            echo "</tr></thead><tbody>"; 
                    foreach ($conn->query($sql) as $row) {
                        $cuscode = $row["REF_NO"]; 
                        echo "<tr>               
                              <td onclick=\"custno1('$cuscode', '$stname');\">" . $row['REF_NO'] . "</a></td>
                              <td onclick=\"custno1('$cuscode', '$stname');\">" . $row['SDATE'] . "</a></td>
                              <td onclick=\"custno1('$cuscode', '$stname');\">" . $row['C_CODE'] . "</a></td>
                              <td onclick=\"custno1('$cuscode', '$stname');\">" . $row['CUS_NAME'] . "</a></td>
                              <td onclick=\"custno1('$cuscode', '$stname');\">" . $row['GRAND_TOT'] . "</a></td>
                            </tr>";
                    }
            ?>
        </table> </div>
