...
if($_POST["Command"]=="save_inv")
{
       ...
       try {
        ...
        $sql="insert into s_crec (CA_REFNO, CA_DATE, CA_CODE, CA_CASSH, CA_AMOUNT, DEPARTMENT, FLAG,pay_type) values('" . trim($_POST["invno"]) . "', '" . $_POST["sdate"] . "','" . trim($_POST["cuscode"]) . "','" . $amou . "','" . $amou . "', '1' , 'PAY','".$_POST["paytype"]."')";
        $result = $conn->query($sql);
        ...
        $sql="UPDATE invpara SET ADVANCE=ADVANCE+1";
        $result = $conn->query($sql);
...
if ($_GET["Command"] == "pass_quot") {
    ...
    if ($rowM = $result->fetch()) {
        ...
        $ResponseXML .= "<cancel><![CDATA[".$rowM["cancel"]."]]></cancel>"; 
 ...
 if ($_POST["Command"] == "cancel_inv") {
    ...
    try {
        ...
        if ($row = $result->fetch()) {
            ...
            $sql5 = "select * from s_adva where C_REFNO ='" . $_POST['invno'] . "' and cancel='1'";
            $result5 = $conn->query($sql5);
            if ($rows5 = $result5->fetch()) { 
                exit("Advance Already Canceled !!!"); 
            } 
